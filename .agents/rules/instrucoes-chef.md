...
* **`/chef/inicio` ([DashboardChef.tsx](file:///Users/viniciussantiago/Matilha/tyt-users/src/pages/DashboardChef.tsx))**: Painel principal do Chef logado. Exibe receita mensal líquida acumulada, total de serviços realizados, calendário resumido de serviços da semana, avaliações dos clientes, atalhos de serviços (Meal Prep, Get Together, Special Service) com badges informativos de pendências de aceite e atalho para aprovações.
* **`/chef/pendentes` ([AprovacoesChef.tsx](file:///Users/viniciussantiago/Matilha/tyt-users/src/pages/AprovacoesChef.tsx))**: Tela de controle para aceite ou recusa de novos serviços atribuídos ao chef (com justificativa de recusa para redistribuição).
...
* **`/chef/meu-perfil` ([EditarCadastroChef.tsx](file:///Users/viniciussantiago/Matilha/tyt-users/src/pages/EditarCadastroChef.tsx))**: Área de atualização do perfil do chef. Permite modificar a biografia, especialidades, idiomas falados, tipos de transporte, foto de perfil (via upload de arquivos JPEG/PNG/WebP até 5MB ou captura ao vivo por câmera com guia visual e cancelamento), e a grade semanal de horários de disponibilidade (Manhã, Tarde e Noite para cada dia da semana). Possui formulário para alteração de senha de acesso.
...
5. **Normalização de Enums**: O Prisma utiliza chaves estritas para `Idioma` (`jp` para Japonês, `br`, `us`, etc.) e `Especialidade` (`Arabe`, `Mediterranea`, `Panificacao`, `Sem_Gluten`, `Doces_e_Sobremesas`). O backend possui normalização automática dessas variações, mas garanta que o frontend envie e consuma as chaves normalizadas (`normalizeSpecialty` e `normalizeLanguage`).
