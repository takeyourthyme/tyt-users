...
* **`/pagamentos` ([HistoricoPagamento.tsx](file:///Users/viniciussantiago/Matilha/tyt-users/src/pages/HistoricoPagamento.tsx))**: Extrato financeiro de todas as cobranças do cliente. Carrega os pedidos reais via `listKitchenOrders({ token })` e enriquece cada item com detalhes do gateway Asaas via `getPaymentDetails(token, id_pagamento)` (expostos em `asaasService.ts`). Exibe: código do pedido (com link para `/meus-pedidos/:code`), tipo de serviço (Meal Prep, Get Together, Special Service), data, forma de pagamento (Cartão com bandeira + últimos 4 dígitos, ou Pix), valor, status do pagamento (Pago, Aguardando, Estornado, Vencido) e botão de comprovante/fatura que abre o recibo do gateway em nova aba (`transactionReceiptUrl` ou `invoiceUrl`). Possui filtros de data inicial/final e busca por código ou tipo de serviço. A tela está ativada no menu lateral do cliente (`AppMenu.tsx`).
...
* **`/minha-conta` ([EditarDadosPessoais.tsx](file:///Users/viniciussantiago/Matilha/tyt-users/src/pages/EditarDadosPessoais.tsx))**: Atualização cadastral (Nome, E-mail, WhatsApp, Endereço, Foto de Perfil via upload ou captura por câmera com espelhamento e guia, Comprovante de Endereço) e alteração de senha (Senha Atual, Nova Senha, Confirmar Nova Senha).
