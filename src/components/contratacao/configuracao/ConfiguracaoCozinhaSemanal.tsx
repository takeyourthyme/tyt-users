...
const categorias = ['Entradas', 'Saladas', 'Pratos Principais', 'Sobremesas', 'Petiscos', 'Aperitivos'];
const preferencias = ['Vegetariano', 'Vegano', 'Sem Glúten', 'Sem Lactose', 'Low Carb', 'Proteína'];
const ingredientes = ['Frango', 'Carne Bovina', 'Peixes', 'Frutos do Mar', 'Queijos', 'Cogumelos'];
...
export const ConfiguracaoCozinhaSemanal: React.FC<Props> = ({
  ...
  const handleAvancar = () => {
    ...
    if (Object.keys(novosErros).length === 0) {
      ...
      onAvancar({
        tamanhoPortacao: tamanhoSelecionado as any,
  ...
  const renderSelectionCards = (title: string, description: string, items: string[], selectedItems: string[], setSelectedItems: (items: string[]) => void) => (
        ...
        {items.map(item => {
          ...
          return (
            <Badge 
              ...
              onClick={() => toggleItem(item, selectedItems, setSelectedItems)}
            >
              {item}
            </Badge>
          );
  ...
  return <div className="space-y-8">
      ...
      <Card>
        <CardContent className="p-6 space-y-6">
          ...
          {/* Categorias */}
          {renderSelectionCards("Categorias", "Selecione os tipos de pratos que você mais gosta", categorias, categoriasSelecionadas, setCategoriasSelecionadas)}

          {/* Preferências Culinárias */}
          {renderSelectionCards("Preferências Culinárias", "Informe suas restrições alimentares ou preferências especiais", preferencias, preferenciaseSelecionadas, setPreferenciasSelecionadas)}

          {/* Ingredientes Preferenciais */}
          {renderSelectionCards("Ingredientes Preferenciais", "Escolha os ingredientes que você mais consome", ingredientes, ingredientesSelecionados, setIngredientesSelecionados)}

          {/* Tipos de Cozinha */}
          {renderSelectionCards("Tipos de Cozinha", "Selecione as culinárias que mais te agradam", tiposCozinha, tiposCozinhaSelecionados, setTiposCozinhaSelecionados)}
